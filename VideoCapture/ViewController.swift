...
class ViewController: NSViewController, AVCaptureFileOutputRecordingDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var extractRegionString: [String] = []//Changed for convenience when writing z-score to csv
...
//                    rep.setColor(annot.color, atX: x, y: y)
            ...
            for annot in view.annotations {
                ...
                extractRegionString.append(regionString)
            }
        }
        else {
...
//        data?.writeToURL(NSURL(fileURLWithPath: "/Users/nathan/Desktop/debug.png"), atomically: false)
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        // update running average and standard deviation calculations (used for z-score)
        if (capturing || monitoring) && !inputBuffer.isEmpty {
            // Pull in new values for this iteration
            numSamples += 1
            var previousInput = newestInput
            newestInput = extractValues
            ...
            if numSamples == 1 { // If there are two or less samples, directly calculate mean/std dev
                ...
                variance = mean.enumerated().map{pow(newestInput[$0] - $1, 2) + pow(previousInput[$0] - $1, 2)}
                m2n = variance
                ...
                var oldMean = mean
                mean = mean.enumerated().map{$1 + (newestInput[$0] - $1)/Float(numSamples)}
                m2n = m2n.enumerated().map{$1 + (newestInput[$0] - oldMean[$0])*(newestInput[$0] - mean[$0])}
                ...
                var oldMean = mean
                mean = mean.enumerated().map{$1 + (newestInput[$0] - oldestInput[$0])/Float(samplesPerWindow)}
                variance = variance.enumerated().map{$1 +
                    ((newestInput[$0] - oldestInput[$0]) * (newestInput[$0] - mean[$0] + oldestInput[$0] - oldMean[$0]))/Float(samplesPerWindow - 1)}
                stdDev = variance.map{sqrt($0)}
                zScores = mean.enumerated().map{(extractValues[$0] - $1)/stdDev[$0]}
                var zScoresOldestInput = zScoresBuffer[zScoresBufferIndex]
                zScoresBuffer[zScoresBufferIndex] = zScores
        ...
        if nil != extractEquation {
            ...
            if let arduino = self.ioArduino, !mode.isEditable() {
                let val = extractEquation!.evaluate(ph)
                
                // normalzie to be an unsigned integer (between 0 and 255)
                let valNorm = UInt8(((val+256)/(256+256))*256)
                
                // update feedback pin
                do {
                    try arduino.writeTo(appPreferences.pinDigitalFeedback, analogValue: valNorm)
                }
                catch {
        ...
        if nil != dataOut {
            ...
            for val in zScores { // Write in z-score values
                sampleString += ",\(val)"
            }
            if nil != extractEquation {
    ...
    func initBMIVariables() {
        ...
        stdDev = [Float](repeating: 0.0, count: extractNames.count)
        oldestInput = [Float](repeating: 0.0, count: extractNames.count)
    ...
    func tableView(_ tableView: NSTableView, objectValueFor tableColumn: NSTableColumn?, row: Int) -> Any? {
        ...
        case "name":
            if row < annotView.annotations.count {
                ...
                result.textField?.stringValue = annotView.annotations[row].name
                return result
            }
            return "ROI"
        case "value":
            if row < extractValues.count {
                let result = tableView.makeView(withIdentifier:(tableColumn?.identifier)!, owner: self) as! NSTableCellView
                result.textField?.stringValue = String(Int(extractValues[row]))
                return result
            }
            return nil
        case "z_score":
